...
from fastapi import APIRouter, Depends, Form, HTTPException, Query , UploadFile , status ,File
...
async def run(query:str , token: str = Depends(JWTBearer()) ,db:AsyncSession = Depends(get_db)):
